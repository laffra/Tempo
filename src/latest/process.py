...
def cpu(pid=-1):
    if pid == 0 or not get_process(pid):
        return 0
    try:
...
def get_owner(pid):
    return get_process(pid).username()
...
def suspend_pid(pid):
    if is_system_process(pid):
        ...
        rumps.alert("Tempo: Suspend Canceled", message)
        return
    try:
...
def execute_as_root(description, command):
    ...
    if not password:
        ...
        window = rumps.Window(
            ...
            "Tempo: To %s, an admin or root Password is needed." % description,
            cancel = "Cancel"
        )
